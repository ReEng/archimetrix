...
public class InterpreterStarter
{
   ...
   private Resource saveTransformedGAST(final ASGAnnotation badSmell)
   {
      ...
      {
         element = annotatedElement.get(0); // any annotated element can be used
      ...
      {
         ArchitecturePreviewPlugin.getDefault().logError("Error occurred during saving transformed GAST", e);
      }
      return newResource;
   }
   ...
   private Map<String, Variable> startInterpretation(final Activity reengineeringStrategy, final List<Parameter> params)
   {
      ...
      {
         ArchitecturePreviewPlugin.getDefault().logError("Error occurred during interpretation.", e);
      }
      return null;
   }
